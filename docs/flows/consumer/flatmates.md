...
- **Domain model:** `flatmates/model.js` - `TAB_MOVE_IN`/`TAB_TEAM_UP`/`normalizeTab`/`tabOf`,
  `ROOM_KINDS`/`roomKindOf`, `PRICE_ROOM`/`PRICE_PERSON`/`priceBasisOf`/`perPersonRent`,
  `OCCUPANCY_*`/`occupancyOf`, `seatsTotalOf`/`seatsOpenOf`/`filledSeatsOf`, `decorateRooms`,
  `bestPerPersonRent`; re-exports the split rules from `flatSplit.js` so view code has one import.
- **Orchestrator:** `flatmates/useFlatmates.jsx` (collections, tabs, interest handlers, `SHARE_OPENER`,
  `refresh`, split-badge reconciliation on mount).
- **Discovery:** `flatmates/useFlatmateDiscovery.jsx` (`emptyFilters`, filters/sort, `activeFilterCount`,
  `raiseHint`, smart search, map focus, `flatmateTabs`, the merged `moveInList` / `teamUpList`).
- **Supply:** `flatmates/useFlatmateSupply.jsx` (`requireSignedIn`, `submitPost`, `submitGroup`,
  `listRoom`, `createGroup`, `onJoin`, `setGroupSeats`/`setRoomSeats`/`setRoomPeople`,
  `reissueAgreement`, `openConsent`, `openVerify`/`onVerified`, `isVerified`).
- **Core data:** `src/lib/data/flatmates.js` - requests/groups CRUD, `evaluateHostEligibility`
  (`MAX_ACTIVE_HOST_SHARES`, `addressFingerprint`, `findAddressClaims`), Ops reviews
  (`enqueueFlatmateReview` / `decideFlatmateReview` / `getFlatmateReviewStatusMap`), owner consent
  (`hasOwnerConsent` / `setOwnerConsent`), seeker verify, interests, and the host inbox
  (`addFlatmateRequest` / `decideFlatmateRequest` / `flatmateReqPendingCount`).
- **Flat split:** `src/lib/data/flatSplit.js` - `canSplitIntoRooms`, `maxRoomsForBhk`, `capBoundsFor`,
  `validateSplit`, `splitFlat`, `unsplitFlat`/`canUnsplit`, `setRoomOccupants`, `splitOccupants`,
  `reconcileSplitVerification`, `ROOM_SHARE_MAX`.
- **Rooms store:** `src/lib/store/listings.js` (`addRoom` / `updateRoom` / `deleteRoom` / `getRooms`,
  `puneNestRoomListings`).
- **Key components:** `PostChooser`, `PostModal`, `GroupModal`, `SplitFlatModal`, `AgreementUpload`,
  `components/auth/AadhaarVerifyModal.jsx` (the shared DigiLocker badge),
  `components/auth/OwnerConsentModal.jsx`, `components/ReportModal.jsx` (`SHARE_REPORT_REASONS`),
  `SeekerCard`/`RoomCard`/`GroupCard`, `FlatmateAlertCard` + `alertCriteria.js`,
  `dashboard/useDashboardData.js` (`decideFlatmateReq`).
- **Data/seed:** seekers/rooms/groups seeds in `flatmates/constants.js`.
- **Route CSS:** `src/styles/routes/flatmates.css`, loaded with the route chunk.
...
- **Discovery:** `GET /flatmates/feed` (tab-aware mixed feed), `GET /flatmates/rooms`,
  `GET /flatmates/groups`, `GET /flatmates/posts`.
- **Seeker posts:** `POST /flatmates/posts`, `PATCH /flatmates/posts/{id}`,
  `DELETE /flatmates/posts/{id}`.
- **Supply:** `POST /flatmates/rooms`, `PATCH /flatmates/rooms/{id}/seats`,
  `PATCH /flatmates/rooms/{id}/occupants`, `POST /flatmates/rooms/{id}/agreement/reissue`,
  `POST /flatmates/groups`, `PATCH /flatmates/groups/{id}/seats`, `DELETE /flatmates/groups/{id}`,
  `POST /flatmates/groups/{id}/owner-consent`.
- **Flat split (Listings tag):** `GET /properties/{propId}/rooms`, `POST /properties/{propId}/split`,
  `DELETE /properties/{propId}/split`.
- **Interest / host inbox:** `POST /flatmates/posts/{id}/interest`,
  `POST /flatmates/rooms/{id}/interest` (carries `ShareIntent`),
  `POST /flatmates/groups/{id}/join`, `GET /me/flatmate-requests`,
  `PATCH /me/flatmate-requests/{id}`.
- **Moderation:** `GET|PATCH /admin/flatmate-reviews`, `PATCH /admin/flatmates/{id}/moderation`,
  `GET|PATCH /admin/group-applications`.
- **Still client-only:** the seeker-side interest map, saved posts and the map focus set are local
  UI state and never serialize.
...
- **Enforce the anti-broker guardrails server-side:** the active-share cap (owner-tier exempt),
  ...
  This applies to **all three** supply paths, including the flat split.
- **Own the verification tiers:** only an Ops-verified property grants `owner` tier; only an
  Ops-verified agreement grants `tenant` tier; the client can't self-assign a host badge. Promote a
  split flat's rooms server-side when the parent listing is approved, instead of relying on the
  owner's session running `reconcileSplitVerification`.
- **Own the occupancy ledger** per flat: clamp `occupants` to `min(ROOM_SHARE_MAX, maxOccupants -
  siblings)` on the server; derive `occupancy`, `flatCommitted`, `flatMax` and `shareMax` rather than
  trusting stored values. Refuse an unsplit once anyone has moved in.
- **Own the Ops review queue** and store the agreement securely (not inline localStorage), gating who
  can read it; write the decision + audit. Require a reason on reject.
...
- **Gate interest/contact** per the seeker's `verifiedContactOnly` and identity rules; persist the
  ...
  dedupe on the server; **persist the room `share` intent** so the owner sees how many people are
  coming (today it survives only in the chat opener).
...
- **Match and fan out flatmate alerts** from `FlatmateAlertCriteria` on the WhatsApp/SMS channels the
  card offers, reusing the saved-search machinery.
