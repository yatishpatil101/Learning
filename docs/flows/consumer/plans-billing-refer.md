...
- **Billing/fees/entitlements:** `src/lib/store/billing.js` (`getFees`, `fee`, `getPlan`, `setPlan`,
  `PAID_OWNER_PLANS`, `isPaidOwnerPlan`, `PLAN_LISTING_LIMITS`, `listingLimit`, `activeListingCount`,
  `canPostListing`, `isBoosted`, `boostListing`, `getServiceOrders`, `addServiceOrder`).
- **Referrals:** `src/lib/store/referrals.js` (`referralCode`, `referralLink`, `getReferralStats`,
  `addReferralInvite/Join/Listing`, `referralListingsTarget=3`, `referralJoinsTarget=1`,
  `referralContactsPerReward=15`, `referralFreeAgreements`, `referralContactsEarned`, `setReferredBy`,
  `getReferredBy`).
- **Provider:** `src/services/providers/mock/savedProvider.js` exposes `getPlan/setPlan/isBoosted/
  boostListing/getServiceOrders/addServiceOrder` as async passthroughs.
- **Pages:** `Plans.jsx` (persona toggle, plan cards, FAQs, rent-agreement fee callout),
  `Checkout.jsx` (`pay`, `alreadyOnThisPlan`, methods), `Refer.jsx` (code/link/share, two reward
  tracks, progress bars), `dashboard/BillingPanel.jsx`.
...
- `GET /me/referral/code`, `GET /me/referral/stats`, `POST /me/referral/invite`, `GET /referrals`
  (admin). **Deltas implied:** join/listing attribution endpoints (server-side, so a friend's action
  on another device credits the referrer), a reward-ledger, and the fraud-signal fields on the
  referral record.

## 11. Backend responsibilities
- **Enforce entitlements server-side:** listing quota (`canPostListing`), featuring/boost eligibility
  (`isPaidOwnerPlan`), and any Seeker Plus contact allowance must be checked on the server on every
  gated action - the client limit is advisory only.
- **Own fees & plan state:** fees come from admin settings; plan changes must be written only after a
  verified payment (real gateway), with an invoice/service-order and audit entry
  (cross-cutting section 4). Never trust a client `setPlan`.
- **Own referral attribution and reward accrual:** credit `joined`/`listed` from the referred user's
  real actions (cross-device), compute `referralFreeAgreements`/`referralContactsEarned` server-side,
  and hold rewards behind fraud review (duplicate Aadhaar, same-device/IP, velocity) before paying
  out - see [`../ops/referrals-fraud.md`](../ops/referrals-fraud.md) (planned).
- **Idempotent invite counting** and rate-limiting so the vanity/abuse vectors the client guards
  against cannot be bypassed by calling the API directly.
