...
- **Data/seed:** `src/data/societies.js` (28 curated + `societies-rera.js` bulk), `localities.js`
  (`LOCALITIES` registry), `localityIntel.js` (`LOC`, `IT_HUBS`).
- **Society state layer:** `src/lib/store/societyAdmin.js` (claims/residents/overlay/suggestions/
  merges), `src/lib/store/society.js` (follows, Q&A), `src/lib/store/community.js`
  (`addCommunitySociety`, `searchSocieties`, `addSocietyLead`), plus contribution/board/whatsapp/
  location/report helpers re-exported through `src/lib/store.js`.
- **Reviews/ratings:** `getEntityReviews` / `addEntityReview` / `entityRating` in `src/lib/store.js`.
- **Key components:** `Societies.jsx` (`enriched`, `results`, `addSociety`, `onFollow`),
  `society/useSocietyHub.js` (controller - `submitReview`, `submitClaim`, `submitResident`,
  `submitSuggest`, `submitContribution`, `submitBoard`, `submitWa`, `submitLocation`,
  `refreshCommittee`), `society/tabs/*`, `SocietySidebar.jsx`, `SocietyModals.jsx`;
  `Locality.jsx` + `locality/helpers.js` (`scoreOf`, `rentOf`, `yieldOf`, `buildTrend`,
  `haversineKm`, `bestForTags`) and `locality/*` cards.
- **Provider seam:** these pages read data helpers directly (societies/localities are static
...
- **Missing but implied:** `GET /societies` (+ filter/sort/paginate per cross-cutting section 5),
  `GET /societies/:slug` (merged read), `POST /me/societies/:slug/follow`, claim (`POST
  /societies/:slug/claim` + admin decision), resident verification (`POST /societies/:slug/residents`
  with OTP + committee/ops decision), detail suggestion, contributions/board/whatsapp/location/report,
  and a `POST /localities/:slug/alert` (or reuse saved-search). See section 11.

## 11. Backend responsibilities
- **Own identity & merges:** resolve slugs, tiers, verified/managed flags and merge redirects
  server-side; never trust a client `status`/`tier`.
...
- **Moderation & audit:** route suggestions/claims/reports/whatsapp/location proposals into ops
  queues; write audit entries and notifications on every decision (cross-cutting sections 4 & 7).
- **Alerts:** persist locality/society follows + saved searches server-side and fire on new matching
  listings.