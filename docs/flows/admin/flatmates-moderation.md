...
- `AdminFlatmates.jsx` - `loadFlatmates()` reads `rawDb()`; `setFlatStatus` / `setAppStatus` write
  through `mutateDb`; `act` / `actApp` add the note + audit; `MOD_BADGE`/`MOD_LABEL` and
  `OWNER_BADGE`/`OWNER_LABEL` map statuses onto the shared `Badge` vocabulary.
- Tables use the shared `ui/Table.jsx` with a `mobileCard` renderer; KPIs use `ui/PageHeader.jsx`
  stats; loading uses `ui/Loading.jsx`.
- **Store split (known inconsistency):** this page reads the mock DB, while the consumer flatmate
  flows write `localStorage` (`lib/data/flatmates.js`, `lib/store/listings.js`). The two are not
  reconciled in the mock.
...
- `PATCH /admin/flatmates/{id}/moderation` - `{ modStatus, note }` for a seeker post or a group.
- `GET /admin/group-applications` / `PATCH /admin/group-applications/{id}` - list and moderate group
  applications (`modStatus` only; the owner's `status` is set through the owner's own endpoint).
- Read paths reuse the consumer discovery endpoints with a moderation-scoped role.

## 11. Backend responsibilities
- **Unify the store.** Seekers, groups, rooms and applications must be one queryable set, so this
  desk can moderate rooms too instead of stopping at the collections the mock DB happens to hold.
- **Make `modStatus` actually suppress supply.** In the mock it is a label; server-side it must
  remove a `flagged`/`removed` post from every consumer feed, the map and the alert matcher.
- **Enforce the two axes separately:** admin may never write the owner's `accepted`/`declined`
  decision, and the owner may never write `modStatus`.
- **Persist the internal note as a first-class record** (author, timestamp, target) rather than a
  prompt string, and keep it out of every consumer-visible payload.
- **Audit every decision** with actor, before/after status and reason; consider a second-approver
  step for `removed`, which is the only action a seeker cannot appeal from the UI.
- **Feed the Flagged count from reports too**, so a post reported by seekers surfaces here without
  waiting for an operator to notice it.
