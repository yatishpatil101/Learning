...
- **Settings page:** `src/pages/admin/AdminSettings.jsx` (`saveSite`, `saveFees`, `saveMovePack`, `saveGeo`,
  `requestAppFlagToggle`, `requestAdminFlagToggle`, `ConfirmDialog`, audit export/clear).
- **Flag panels:** `settings/AppFlagsPanel.jsx`, `settings/AdminFlagsPanel.jsx`, `settings/MapsGeoPanel.jsx`;
  admin-flag state via `src/context/AdminFlagsContext.jsx`.
- **Team page:** `src/pages/admin/AdminTeam.jsx`; RBAC in `src/lib/adminModules.js` + `src/lib/permissions.js`;
  role labels `src/lib/auth.js`.
- **Services:** `getSettings`/`updateSettings` (`src/lib/mockApi/collections.js`); `listTeamMembers`, `saveTeamMember`,
  `setTeamMemberStatus`, `deleteTeamMember`, `listCustomRoles`, `saveCustomRole`, `deleteCustomRole`, `getTeamMemberByMobile`
  (`src/lib/mockApi/team.js`); `listAudit`/`clearAudit`/`logAudit` (`src/lib/mockApi/audit.js`).
- **Seed:** `src/data/settings.json` (`site`, `fees`, `movePack`, `flags`, `adminFlags`, `permissions`, `customRoles`);
...
- Users (Admin) (section 5) - adjacent to internal-account management.
- **Deltas implied but not in the contract yet:** internal team CRUD (`GET/POST/PATCH/DELETE /admin/team`),
  custom-role CRUD (`.../roles`), a role/permission resolution endpoint, and a member status endpoint - none exist yet.

## 11. Backend responsibilities
- **Enforce RBAC on the server:** module authorization, `adminOnly` protection, verify-only sub-scope, and the
  last-active-admin guardrails must be authoritative - the client model is explicitly UX-only.
- **Authorize + validate settings writes** (fees, geo, flags): only a super-admin may change them; validate ranges/formats.
- **Own the audit log** with a trusted actor identity and immutability (clear should be privileged/append-only, not a silent wipe).
- **Protect internal-account PII** (staff mobiles/emails) and keep the internal `team` store separate from consumer `users`.
- **Emit config-change events** so dependent services (geo, flags, fees) refresh consistently.
