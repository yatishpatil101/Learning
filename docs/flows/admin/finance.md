...
- **Page + handlers:** `src/pages/admin/AdminFinance.jsx` (`KPIS`, `pct`, `FlowRow`, filters,
  `doRevenueExport`, `doTxExport`, detail modal).
- **Aggregation:** `src/lib/data/finance-admin.js` - `buildTransactions()`, `buildRevenueSeries(24)`, `rentFeeRevenue(db)`.
- **Settings/db access:** `src/lib/mockApi.js` re-exporting `getSettings()` (`mockApi/collections.js`)
  and `rawDb()` (`mockApi/core.js`).
- **Seed:** `src/data/settings.json` (`fees`), `src/data/analytics.json` (`revenue[]`),
  plus `deals.json`, `tickets.json`, `properties.json` (featured), `users.json` for the ledger; `rentFeeLedger` is runtime-only.
- **Note:** `src/services/financeService.js` / `providers/mock/financeProvider.js` are a **different**
...
- `GET /fees` (section 33) and `GET /admin/settings` (section 29) - the fee schedule + `gstPercent`.
- `GET /rent-payments` (section 17) - source of the rent-pay platform-fee ledger (`rentPayPercent`).
- **Deltas implied but not in the contract yet:** a payouts endpoint (`GET /admin/finance/payouts`),
  a GST/tax summary, and subscription counts by plan (owner/seeker) so the split is a real query,
  not a 55/45 guess. Revenue-series response needs `{ month, subscriptions, services, featured }` per row.

## 11. Backend responsibilities
- **Authorize** the finance module (admin/manager) for every read; never expose PII-bearing party
  names without authorization.
- **Own the ledger:** an immutable, gateway-backed transactions table (id, date, party, type, amount,
  status, method, GST) - not synthesised from deals/tickets/featured.
- **Compute money server-side:** revenue totals, MoM deltas, MRR, ARPU, GST collected, partner payouts,
  platform commission and net-retained - the client must not be trusted to split 55/45 or 65/35.
- **Subscription accounting:** real active-plan counts per plan and their prices, replacing the modelled split.
- **Refund/payout workflows:** add maker-checker + audit for any money movement (not present today).
