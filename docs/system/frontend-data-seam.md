...
| `recentSearch` | `recentSearchService.js` | mock + **http** | Live: the signed-in "resume your search" rail — `GET /me/recent-searches` and `PUT /me/recent-searches`. **The only domain whose provider is chosen by who is asking, not by configuration.** The service branches on the session: signed-in goes to the provider, anonymous stays on `lib/localPrefs.js`, because a trail nobody signed in to create has no account to belong to. That branch lives in the service and not in the three call sites (`HeroSearch`, `HeroSearchPanel`, `Dashboard`) and not in the http provider, which must stay a dumb wire mapper. A mock provider exists anyway even though the anonymous path never reaches it, so `createProvider('recentSearch')` never hits `getLoader`'s live-only throw and dev startup validation passes. **Dedupe is by normalised URL, never by label** — query parameters sorted, so `?deal=rent&loc=baner` and `?loc=baner&deal=rent` are one chip, while "3 BHK in Baner" with and without a budget filter stay two rows. The old client keyed on the label and was silently losing the second one. `at` is ISO on the wire and epoch millis in the browser, the same conversion `notification` needs. The **cap of six is enforced on write**, and `PUT` answers with the whole rail, so no client ever models the eviction. The stored URL is handed back as a link the account's own UI invites the user to click, which makes a permissive path rule a stored-redirect primitive: the server takes an **allowlist of exactly the two search routes**, so adding a third search page means adding it here deliberately. Backed by a new `engagement.history` package rather than `engagement.search`, which is `SavedSearch` alerts — a saved search is a standing subscription, a recent search is a footprint, and folding them together would have given the footprints an alert frequency (D248) |
...
| `dismiss` | client tombstones (`dzDismissedNotifs`) | no `DELETE /notifications/{id}` |
...
slice and were never called once. Following stayed `dzFollowedSocieties`, a localStorage array read
...
those follows are kept in `dzLocalSocietyFollows` and retried on every load. Not a concession: the
...
- `dzContactsUsed:<mobile>` — the counter, incremented by the same code that read it.
- `referralBonusContacts()` — the allowance top-up, derived from `dzReferralStats`, which the browser
  also wrote.
...
`rent_mandates`, `payout_accounts`) went in V127. Draazy is not a payments business, and a dormant
...
own note about a home they rent — usually **not** a Draazy listing, which is why `tenant_rentals`
...
| **Three endpoints added**: `PATCH /me/saved-searches/{id}`, `GET /me/properties/{propId}/boost`, `GET /admin/reviews` | Each was a feature whose UI could write but never read. `toggleSearchAlert` in the mock provider now has a real counterpart; the boost UI can render its own state; the review moderation queue is reachable. **"Reachable" turned out to be the operative word for the last of the three: `GET /admin/reviews` and `PATCH /reviews/{id}/status` are both built and tested, and as of 2026-08 nothing in `frontend/src` calls either — `AdminContent.jsx`'s Reviews tab still moderates the browser copy. See `tasks/DECISIONS-NEEDED.md` item 30.** |
