...
| `serverSideSearch` | **Batch H** appends **p5140–p5149**, ten approved Buy flats in **Wagholi**, and exists for one reason: until it landed, the largest search in the seed was `deal=buy` with **19 rows** against a page size of **24**, so *"the server filtered the catalogue and sent page 1"* and *"the server sent everything and the browser filtered it"* produced identical output. A regression from server-side search back to client-side slicing would have gone green. Three properties of the rows are load-bearing. **They are the oldest rows in the catalogue** (January, against everything else's April–August), so both `newest` and relevance put them last and all nineteen pre-existing Buy listings stay on page 1 exactly where they were — every spec that opens `/listings?deal=buy` looking for a particular card still finds it. **They are all in one locality that previously held a single approved Buy listing** (deliberately not Magarpatta, which `live-location-recovery.spec.js` needs empty so a locality+near-a-place query has to auto-relax), and sitting last behind nineteen older rows they *straddle the page boundary*: five land on page 1, five on page 2. `deal=buy&loc=wagholi` is therefore the query a browser filtering the twenty-four rows it happens to be holding answers with **6** - five on page 1 plus the pre-existing open plot p5124 - and a server answers with **11** — that gap is why this is ten rows in one locality rather than ten scattered, since scattered, each locality would have fit on the page it was already on. **They carry no trust badge on either column**: `owner_verified` *and* `ownership_verified` are both false, so `deal=buy` returns **29 listings of which 19 are verified**. Both halves matter — `verifiedElements` counts `owner_verified OR live ownership verification`, so leaving `owner_verified` set would have kept the tally equal to the total no matter what the ownership column said - and `owner_verified` is not settable per listing at all, since the seed derives it from the owner's `aadhaar_verified`, which is why these rows belong to Isha Mehta (`b05422ba`) rather than the usual seed owner, and while every Buy listing was verified a count describing only the current page was indistinguishable from one describing the whole match. They state **nothing optional** — no age, floor, facing, society or conveyance flag — so the exact-slug assertions in `live-listing-attributes` are untouched and these rows are covered by that spec's *"listings that state nothing are excluded from narrowed searches"* rule for free. Note the seed is `ON CONFLICT DO NOTHING` with **no conflict target**, so it never updates an existing row: correcting one of these means `DELETE`ing it before replaying the file. Covered by `consumer/search/live-server-side-search`. | `properties` |
