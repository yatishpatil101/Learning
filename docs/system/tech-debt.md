...
| D78 | **`/me/contact-requests` cannot be paged until a server-side pending count exists** — the endpoint's own Javadoc records that the UI derives its "waiting on you" badge by filtering the full array client-side, because no `pendingContactCount` endpoint exists. Page it as-is and the badge silently under-counts past page one. This is the inverse of §5.1's "a client-side pager is a smell": a client-side *aggregate* blocking a server-side pager. Notable because contact requests are the highest-volume inbound surface on a marketplace, so it is simultaneously the one that most needs paging | design | Med | add the count endpoint first, then page — in that order |
