...
const PROP = 'P5013';
const UNREVIEWED = 'P5100';

/** Ratings 5/4/3 → avg 4.0, one review on each of the top three bars, none on 1★ or 2★. */
const SEED = [
  { id: 'rv-a', user: 'Anita R', rating: 5, text: 'Great locality.', at: '2026-01-05', categories: { locality: 5, condition: 4 }, recommend: true, context: 'visit' },
  { id: 'rv-b', user: 'Bharat K', rating: 4, text: 'Fair value.', at: '2026-01-04', categories: { locality: 4 }, recommend: true, context: 'tenant' },
  ...
  { id: 'rv-c', user: 'Chetan S', rating: 3, text: 'Average.', at: '2026-01-03', categories: {}, recommend: null, context: null },
];

async function openReviews(page, id, reviews) {
  if (reviews) {
    await page.addInitScript((payload) => {
      window.localStorage.setItem('puneNestPropReviews', JSON.stringify(payload));
    }, { [id]: reviews });
  }
...
test('the rating summary reports the seam\'s aggregate, not a tally of the cards below it', async ({ page }) => {
  const section = await openReviews(page, PROP, SEED);
