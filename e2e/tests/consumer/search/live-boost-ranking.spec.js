...
/* A boost buys position in the default order only, never in one the buyer asked for by name, and never
 * silently. Wagholi, because p5145's nine siblings differ in nothing else the ranker can see.
 */
...
/**
 * Every `toHaveCount(rows.length)` here rests on the whole locality fitting on one page — a row count other
 * live specs can grow. Unstated, the 25th Wagholi listing reads as a ranking defect; asserted, it names itself.
 */
const PAGE_SIZE = 24;
...
test.describe('Paid placement (D59)', () => {
  ...
  test('exactly one card discloses that its position was bought', async ({ page }) => {
    const rows = (await wagholi()).content;
    assertFitsOnePage(rows);
    // Re-derived from the same read rather than written as `1`, so a second seeded boost fails as "the grid
    // disclosed fewer promotions than the API reported" rather than as an untraceable literal.
  ...
  test('an explicit price order is the buyer\'s — money does not outrank it', async ({ page }) => {
    ...
    // Disclosure survives the reorder. The whole locality fits on one page, so a missing badge means it
    // failed to render rather than that it was paged away.
    await expect(page.getByText(/^Promoted$/i)).toHaveCount(boosted);
