...
/* Listings "Map view" marker interaction. On the listings map, clicking a price
   marker no longer opens a leaflet popup — it opens the slide-in Map Detail Panel
...
test('clicking a map marker opens the detail drawer, not a leaflet popup', async ({ page }) => {
  ...
  // The map is area-first: it only renders once 1–5 localities are focused, so we
  ...
  // Target the villa's distinctive marker (₹2.73Cr) rather than .first() — markers can
  ...
  // The listings map opens the drawer — no map InfoWindow popup must appear.
...
test('map drawer reflects the marked property (title, price + facts match the pin)', async ({ page }) => {
  await injectStock(page, [saleListing]);
  await page.goto('/listings?deal=buy&view=map&loc=Baner');
  // The injected villa's marker label is ₹2.73Cr (27300000 / 1e7, sale ≥ 1 Cr).
  const marker = page.locator('.price-marker', { hasText: '₹2.73Cr' }).first();
  await marker.waitFor({ timeout: 20000 });
  await marker.click();
  ...
  // Facts come from the real listing fields — 3 bedrooms, real baths (3), area.
