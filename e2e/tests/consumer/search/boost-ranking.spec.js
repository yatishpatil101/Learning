...
/* A boost buys position in the default order only. Wagholi, because p5145's siblings never rank
 * below it on merit, so first place is the promotion's doing or nothing at all. */
...
/* Every `toHaveCount(rows.length)` here rests on the whole locality fitting on one page — unstated,
 * the 25th Wagholi listing would read as a ranking defect. */
const PAGE_SIZE = 24;
...
test.describe('Paid placement (D59)', () => {
  test('the server promotes exactly one listing, and its rivals are otherwise indistinguishable', async () => {
    ...
    // owner, same locality, same posting date, same trust badges, no boost.
