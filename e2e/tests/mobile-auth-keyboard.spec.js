...
/* Sign-in bounces an *unregistered* number straight to /signup (see Signin.sendOtp),
   so a spec that wants the OTP step has to start from a number the app knows. Same
   seed trick auth-flow.spec.js uses. */
const REG_MOBILE = '9876500033';

test.beforeEach(async ({ page }) => {
  await page.addInitScript(([mob]) => {
    try {
      localStorage.setItem('puneNestUsers', JSON.stringify([
        { name: 'Reg User', mobile: mob, email: '', role: 'buyer', joinedAt: Date.now() },
      ]));
