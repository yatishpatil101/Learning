...
/* `(pointer: coarse)` / `(hover: none)` rules cannot be disproved from the `mobile` project —
   Playwright's device descriptors set hasTouch regardless of viewport. This runs on desktop. */
...
test.describe('Mobile-only rules do not leak to desktop', () => {
  ...
  test('no bottom inset is reserved on desktop', async ({ page }) => {
    /* The consent bar reserves its real height at every width and is deliberately NOT mobile-only,
       so seeding a choice leaves only the mobile bottom-nav inset in the reservation. */
  ...
  test('the card lift survives the hover gate', async ({ page }) => {
    /* The other half of the `@media (hover: hover)` gate: a mouse must still get the lift.
       `.cat-card`, because the home Featured card's `list-reveal` animation pins its transform. */
    await page.goto('/');
  ...
  /* `networkidle` is not a readiness gate on a client-rendered app, and most of this file asserts
     absence — an early assertion is not flaky, it is vacuous. Each gate is a positive fact. */
  ...
  test('the wizard step actions are not sticky on desktop', async ({ page, login }) => {
    /* Signed in because `/list-property` is behind the auth gate, and as a *new* owner because it
       is also behind the listing paywall — the seeded owner gets the upgrade prompt instead. */
  ...
  test('the property gallery keeps its thumbnail strip and fixed-height hero', async ({ page }) => {
    ...
    // Waiting for the first card rather than counting after `networkidle`: zero cards at that
    // instant means "too early", not "no listings", and would skip the test either way.
...
/* Phase 3 systems are all mobile-only, and cannot live in a mobile-*.spec.js: that project runs
   with hasTouch and so can never disprove a (pointer: coarse) / (hover: none) rule. */
test.describe('Desktop non-leak — phase 3', () => {
  ...
  /* No desktop assertion for the pressed-state feedback: Chrome already computes
     -webkit-tap-highlight-color as transparent on a pointer device, so it proves nothing. */
...
test.describe('Desktop non-leak — landscape & dynamic type (phase 4)', () => {
  /* The landscape block is guarded three ways: (orientation: landscape) AND (max-height: 500px)
     AND (max-width: 1023.98px). These prove each guard holds rather than reading the query. */
  ...
  test('the phone-only pill shrink does not reach desktop', async ({ page }) => {
    /* .dz-topbar__pill and .dz-topbar__icon-box only take effect below sm; above it the pills must
       fall back to their intrinsic padding-driven size. */
    await page.setViewportSize({ width: 1440, height: 900 });
    await page.goto('/');
    const r = await page.evaluate(() => {
    ...
    /* `null` means the element is not in the DOM, which an `if (r.pillH)` guard would silently
       treat as "nothing to check" — it is in fact a total layout failure. */
...
test.describe('Home featured-first is mobile-only', () => {
  ...
  test('the inline search panel is still in the hero', async ({ page }) => {
    // Mobile drops it entirely and sends the Search tab to /listings; desktop must be untouched.
    await page.goto('/');
  ...
  test('the verified-homes proof stays inside the Featured header block', async ({ page }) => {
    // Mobile lifts this line out of the header to caption the rail. Desktop keeps it stacked under
    // the heading and subtitle, inside the header column.
    await page.goto('/');
    await expect(page.locator('.property-card').first()).toBeVisible();
    const r = await page.evaluate(() => {
  ...
  test('the desktop vertical rhythm keeps its original per-section values', async ({ page }) => {
    // Mobile collapses every section-header gap onto one token; desktop keeps the hand-authored
    // values the paired sm:mb-* utilities restore.
    await page.goto('/');
    await expect(page.locator('.property-card').first()).toBeVisible();
    const r = await page.evaluate(() => ({
