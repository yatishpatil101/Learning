...
import { loginAsOwner, loginAsAdmin, loginAsStaff } from '../../helpers/auth.js';
...
const AUTHED_ROUTES = [
  ['/dashboard', loginAsOwner],
  ['/messages', loginAsOwner],
];
...
/* The staff surface. Field ops and moderators work these screens on a phone in
   ...
   fixture into a mobile spec. `helpers/auth.js` already had one
   (`touch-targets-p3.spec.js` uses it), so the deferral was stale. */
const STAFF_ROUTES = [
  ['/admin', loginAsAdmin],
  ['/ops', (page) => loginAsStaff(page, 'Rental')],
];
...
async function renderedSweep(page, route) {
  ...
  const { candidates, textLength, undersized } = await undersizedTargets(page, MIN_TAP, EXEMPT);
  expect(
    candidates,
    `${route} rendered only ${candidates} interactive elements (body text ${textLength} chars) — `
    + 'that is an unrendered or broken page, not a clean one. Refusing to report a pass.',
  ).toBeGreaterThanOrEqual(MIN_CANDIDATES);

  expect(undersized, `undersized targets on ${route}:\n${JSON.stringify(undersized, null, 2)}`).toEqual([]);
}
...
test.describe('Mobile tap-target sweep — staff surface', () => {
  for (const [route, signIn] of STAFF_ROUTES) {
    test(`every interactive element on ${route} clears ${MIN_TAP}px`, async ({ page }) => {
      await withConsent(page);
      /* The quick-access sign-in lands on the console itself, so this is a
         same-origin re-navigation rather than a fresh load. */
      await signIn(page);
      await page.goto(route);
      await renderedSweep(page, route);
    });
