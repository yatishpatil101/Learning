...
async function signedIn(page) {
  await page.addInitScript(() => {
    try {
      localStorage.setItem('puneNestUser', JSON.stringify({ name: 'Test Owner', mobile: '9876500000', role: 'owner', loginAt: Date.now() }));
...
test.describe('Mobile listing wizard', () => {
  test('the step actions are pinned, not parked at the end of the form', async ({ page }) => {
    await signedIn(page);
  ...
  test('an invalid Next scrolls the first bad field into view and focuses it', async ({ page }) => {
  ...
  test('every field that fails is marked, not just the first', async ({ page }) => {
    await signedIn(page);
    await openWizard(page);
  ...
  test('the draft survives a reload the way an evicted tab would', async ({ page }) => {
