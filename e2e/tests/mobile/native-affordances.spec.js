...
test.describe('Haptics', () => {
  test('saving a listing ticks', async ({ page }) => {
    await consent(page);
    await stubVibrate(page);
    await page.addInitScript(() => {
      localStorage.setItem('puneNestUser', JSON.stringify({ name: 'Test Buyer', mobile: '9876500001', role: 'buyer', loginAt: Date.now() }));
      localStorage.setItem('puneNestUsers', JSON.stringify([{ name: 'Test Buyer', mobile: '9876500001', role: 'buyer' }]));
    });
  ...
  test('a user who asked for less motion is never buzzed', async ({ page }) => {
    await consent(page);
    await stubVibrate(page);
    await page.addInitScript(() => {
      localStorage.setItem('puneNestUser', JSON.stringify({ name: 'Test Buyer', mobile: '9876500001', role: 'buyer', loginAt: Date.now() }));
  ...
  test('the OS-level reduced-motion setting is honoured too', async ({ page }) => {
    await consent(page);
    await stubVibrate(page);
