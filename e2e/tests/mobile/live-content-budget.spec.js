...
test.describe('Mobile reach and legibility', () => {
  ...
  test('the flatmates VERIFIED badge is readable and stays in its card', async ({ page }) => {
    await consent(page);
    /* `?view=team-up` rather than the bare page: the default `move-in` tab carries rooms
       and groups that already hold a flat, and the seeker badge this test is about only
       renders on a SeekerCard. Landing on the default tab found no badge at all — a
       missing element reads the same as a broken one, so the tab has to be named. */
