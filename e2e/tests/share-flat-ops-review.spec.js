...
/* Ops agreement-review queue. Tenant-tier flat-share posts are self-declared, so
...
async function seedOps(page, groups, reviews) {
  await page.addInitScript((args) => {
    ...
    if (!localStorage.getItem('puneNestShareGroups')) localStorage.setItem('puneNestShareGroups', JSON.stringify(g));
    if (!localStorage.getItem('puneNestShareReviews')) localStorage.setItem('puneNestShareReviews', JSON.stringify(r));
...
test('Ops approves a tenant declaration → consumer card shows Ops-verified', async ({ page }) => {
  ...
  await page.goto(`${BASE}/ops/share-review`);
