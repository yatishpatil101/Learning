...
test('the candidates queue holds the unverified community society and not the verified one', async ({ page, login }) => {
  ...
  await expect(rows(page).filter({ hasText: SEEDED_CANDIDATE })).toHaveCount(1);
  ...
  await expect(rows(page).filter({ hasText: SEEDED_VERIFIED })).toHaveCount(0);
});
...
test('a candidate with no recorded provenance claims none', async ({ page, login }) => {
  ...
  const row = rows(page).filter({ hasText: SEEDED_CANDIDATE });
  await expect(row).toHaveCount(1);
...
test('merging a duplicate takes it off the queue without deleting it, and undoes cleanly', async ({ page, login }) => {
  /* The duplicate is minted here; the survivor is a catalogue society. That asymmetry is not
     tidiness — the merge picker still searches the client catalogue (`searchSocieties`), so a
     survivor minted over the API a second ago is not in it. Recorded rather than worked around:
     the picker is the last store reader on this screen and is named in `tasks/todo.md`. Using a
     catalogue name here means this spec exercises the picker as an operator actually meets it. */
  ...
  await rows(page).filter({ hasText: dupe }).getByRole('button', { name: 'Merge' }).click();
  ...
  await expect(rows(page).filter({ hasText: dupe })).toHaveCount(0);
  ...
  await expect(rows(page).filter({ hasText: dupe })).toHaveCount(1);
});
