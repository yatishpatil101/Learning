...
test.describe('Verification Queue', () => {
  ...
  test('review modal close button works', async ({ page }) => {
    ...
    if (await reviewBtn.isVisible()) {
      await reviewBtn.click();
      await page.waitForTimeout(800);
      /* exact: true — Modal renders its dismiss icon as `aria-label="Close {title}"`
         (see Modal.jsx), so a loose 'Close' also matches "Close Verify property".
         Both are legitimate; the footer button is the one this test means. */
...
test.describe('Flag modal', () => {
  ...
  test('requires reason before submission', async ({ page }) => {
    ...
    await flagBtn.click();
    await page.waitForTimeout(500);
    // Click the submit button (the one in the modal footer). exact: true because the
    // modal is *titled* "Flag listing", so its dismiss icon is aria-labelled
    // "Close Flag listing" and a loose match would hit both.
...
test.describe('View modal', () => {
  ...
  test('shows close button and open public page link', async ({ page }) => {
    ...
    await viewBtn.click();
    await page.waitForTimeout(500);
    // exact: true — see the note on the Verify modal above; the dismiss icon also
    // carries "Close" as a prefix of its aria-label.
