...
test('admin Localities queue lists a pending community locality and Verify promotes it', async ({ page }) => {
  ...
  const row = page.getByText('Testmint Nagar', { exact: true });
  await expect(row).toBeVisible({ timeout: 5000 });

  await page.getByRole('button', { name: 'Verify' }).first().click();
  await expect(page.getByText(/promoted to a curated locality/i)).toBeVisible({ timeout: 5000 });
  // It leaves the pending queue.
  await expect(page.getByText('Testmint Nagar', { exact: true })).toHaveCount(0, { timeout: 5000 });
