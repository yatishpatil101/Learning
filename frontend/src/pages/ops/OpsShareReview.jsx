...
import { getShareReviews, decideShareReview } from '../../lib/data/shareFlat.js';
...
// Open an uploaded agreement in a blank tab. Only base64 image/PDF data URLs are
// allowed and the URL is assigned to the tab's location directly (never injected
// into HTML), so a tampered store value can't smuggle markup/script into Ops.
const AGREEMENT_VIEW_RE = /^data:(image\/[a-z0-9.+-]+|application\/pdf);base64,/i;
const openAgreement = (dataUrl) => {
  if (!AGREEMENT_VIEW_RE.test(dataUrl || '')) return;
  const w = window.open('', '_blank', 'noopener,noreferrer');
  if (w) w.location.href = dataUrl;
};

/* Ops agreement-review queue. A sitting tenant's "I have a registered rent
   agreement" is self-declared, so tenant-tier flat-share posts (and any address a
...
export default function OpsShareReview() {
  const { toast } = useToast();
  const [all, setAll] = useState(() => getShareReviews());
  const [tab, setTab] = useState('pending');
  const [rejecting, setRejecting] = useState(null); // review id being rejected
  const [reason, setReason] = useState('');

  const reload = () => setAll(getShareReviews());

  const approve = (r) => {
    decideShareReview(r.id, 'approved');
  ...
  const confirmReject = (r) => {
    ...
    decideShareReview(r.id, 'rejected', clean);
  ...
  return (
    <div>
      <PageHeader title="Flat-share Verification" subtitle="Verify tenant replacement claims before they earn seekers' trust." />
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {rows.length ? rows.map((r) => (
              ...
              <tr><td colSpan={6} className="p-10 text-center text-sm text-gray-500">No flat-share reviews here 🛏️</td></tr>
            )}
          </tbody>
        </table>
      </div>
