...
import { localities, ownershipOptions, agreementOptions, lockinOptions, noticeOptions, commercialAgreementOptions, commercialLockinOptions, commercialNoticeOptions } from './constants.js';
import SocietySelect from './SocietySelect.jsx';

const LocationPricingStep = ({
  form, set, errors, isLand, isCommercial, money, legacyAddress,
  ...
  const unitLabel = commercial ? t('listProperty.fields.unitShopNoReq') : t('listProperty.fields.flatUnitNoReq');
  ...
  const projectLabel = land ? t('listProperty.fields.projectLayoutName') : commercial ? t('listProperty.fields.buildingComplexNameReq') : t('listProperty.fields.buildingSocietyNameReq');
                ...
                <div className="lp-step">
                  ...
                  {legacyAddress && (
                    <div className="mb-6 rounded-xl border border-white/10 bg-white/[0.03] p-4">
                      ...
                      <p className="mt-2 text-xs text-gray-400">
                        {land
                          ? t('listProperty.edit.legacyLandAddress', { defaultValue: 'The separate address details were never saved. Leave them blank to keep this address, or enter a complete replacement including the project/layout name below.' })
                  ...
                  <div className="mb-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div><label className={lbl}>{t('listProperty.fields.locality')}</label><LocalitySelect value={form.locality} onChange={(v) => onLocalityChange(v)} onSelect={(sel) => onLocalityChange(sel.name, sel)} placeholder={t('listProperty.ph.selectLocality')} options={localities} dataErr="locality" invalid={!!errors.locality} /><FieldError show={!!errors.locality}>{t('listProperty.err.locality')}</FieldError></div>
                      {/* Keep unit and wing paired on mobile without changing the desktop grid. */}
                    {!land && (
