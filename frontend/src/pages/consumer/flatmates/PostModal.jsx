      ...
      <div className="glass rounded-3xl w-full max-w-xl p-6 sm:p-7" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={submitPost} className="space-y-4" ref={postFormRef}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div><label className="block text-xs font-medium text-gray-400 mb-1.5">{tr('flatmates.flatmatePrefWith')}</label><NativeSelect value={post.flatPref} onChange={(e) => setPost({ ...post, flatPref: e.target.value })} className="field w-full rounded-full px-4 py-2 text-sm"><option value="any">{tr('flatmates.optAnyone')}</option><option value="women">{tr('flatmates.optWomenOnly')}</option><option value="men">{tr('flatmates.optMenOnly')}</option></NativeSelect></div>
            <div><label className="block text-xs font-medium text-gray-400 mb-1.5">{tr('flatmates.roomPreference')}</label><NativeSelect value={post.roomPref} onChange={(e) => setPost({ ...post, roomPref: e.target.value })} className="field w-full rounded-full px-4 py-2 text-sm"><option value="any">{tr('flatmates.optNoPreference')}</option><option value="private">{tr('flatmates.optPrivateRoom')}</option><option value="shared">{tr('flatmates.optSharedRoom')}</option></NativeSelect></div>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
