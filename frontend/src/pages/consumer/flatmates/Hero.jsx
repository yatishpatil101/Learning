...
export default function Hero({ user, isVerified, openVerify }) {
  ...
  return (
    <div className="glass rounded-3xl p-4 sm:p-6 mb-3 sm:mb-4 reveal relative overflow-hidden">
      ...
      <div className="relative flex flex-col lg:flex-row lg:items-center justify-between gap-3 sm:gap-5">
        <div className="max-w-2xl">
          ...
          {/* Supporting proof, not a headline, so the icons shrink on phones. Do not shrink the
              type: styles/min-font-size.js floors every font-size at 12px. */}
          <div className="flex flex-wrap items-center gap-x-3 sm:gap-x-4 gap-y-1 sm:gap-y-1.5 mt-2 sm:mt-3 text-xs text-gray-400">
        ...
        {/* The `user &&` guard sits on the WRAPPER: an empty flex item still takes the parent's
            `gap-3 sm:gap-5`, padding the hero for a guest — the default state here. */}
        {user && (
          <div className="flex flex-row flex-wrap sm:flex-nowrap sm:flex-col items-stretch sm:items-start lg:items-stretch gap-2 flex-shrink-0 w-full sm:w-auto">
            {isVerified
              ? <div className="flex-none inline-flex items-center justify-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold text-emerald-300 bg-emerald-500/10 border border-emerald-500/25"><Icon name="badge-check" className="w-4 h-4" /> {t('flatmates.verifiedSeeker')}</div>
              : <Button onClick={openVerify} variant="secondary" icon="shield-check" className="flex-none">{t('flatmates.getVerified')}</Button>
            }
          </div>
        )}
      </div>
