...
function openDoc(d, toast) {
  ...
  try {
    ...
    if (!w) toast('Allow pop-ups to view the document', 'info');
...
export default function DocVault({ mobile, propId, onChange }) {
  ...
  const onPick = (e) => {
    ...
    const finish = (dataUrl) => {
      ...
      sync();
      toast('Document added to your passport', 'success');
    };
  ...
  const remove = (id) => { deleteDocument(mobile, propId, id); sync(); toast('Document removed', 'info'); };
  ...
  return (
    <div className="glass-card rounded-2xl p-6">
      <div className="flex items-center gap-2 mb-1">
        ...
        <h2 className="text-lg font-bold text-white">Document passport</h2>
      </div>
      <p className="text-gray-400 text-sm mb-5">Your deeds, tax receipts and agreements — private, in one place. Only you can see them.</p>
      ...
      {docs.length ? (
        <>
          <div className="flex items-center gap-2 mb-3 text-xs text-gray-400">
            ...
            <span><span className="text-gray-200 font-semibold">{docs.length}</span> {docs.length === 1 ? 'document' : 'documents'} across <span className="text-gray-200 font-semibold">{covered}</span> of {CATEGORY_NAMES.length} categories</span>
          </div>
            ...
            {groups.map((grp) => (
              <div key={grp.cat}>
                <div className="flex items-center gap-1.5 mb-1.5 px-0.5">
                  ...
                  <span className="text-[11px] font-semibold uppercase tracking-wide text-gray-400">{grp.cat}</span>
                  ...
                  {grp.items.map((d) => (
                    <li key={d.id} className="rd-cell flex items-center gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-gray-500 text-[11px]">{d.size ? formatSize(d.size) : ''}{d.dataUrl ? '' : (d.size ? ' · metadata only' : 'metadata only')}</p>
                      </div>
                      {d.dataUrl ? (
                        <button onClick={() => openDoc(d, toast)} aria-label={`View ${d.name}`} className="p-2 rounded-lg text-gray-500 hover:text-brand-teal-3 hover:bg-brand-teal/10 transition-all">
                          <Icon name="eye" className="w-4 h-4" />
                        </button>
                      ) : null}
                      <button onClick={() => remove(d.id)} aria-label={`Delete ${d.name}`} className="p-2 rounded-lg text-gray-500 hover:text-rose-300 hover:bg-rose-500/10 transition-all">
        ...
        <p className="text-gray-500 text-sm text-center py-6">No documents yet. Add your first — a sale deed or tax receipt is a good start.</p>
      )}
    </div>
  );
}
