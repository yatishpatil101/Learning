  ...
  return (
    <div className="glass-card rounded-2xl p-5 sm:p-6 reveal">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
        <div><h2 className="text-lg font-bold text-white">{t('locality.trendTitle')} <span className="text-gray-500 text-sm font-normal">{t('locality.trendUnit')}</span></h2><p className="text-gray-500 text-xs mt-0.5">{t('locality.trendSub')}</p></div>
      ...
      <div className="flex flex-wrap gap-x-5 gap-y-2 mt-4 text-xs">
        ...
        <span className="flex items-center gap-2 text-gray-400"><span className="w-3 h-0.5 rounded bg-[#6366f1]" /> {t('locality.trendPuneAvg')}</span>
        <span className="flex items-center gap-2 text-gray-400"><span className="w-3 border-t border-dashed border-amber-400" /> {t('locality.trendForecast')}</span>
      </div>
    </div>
  );
}
