  ...
  return (
    <div className="reveal">
      <div className="flex items-center justify-between gap-3 mb-3">
        <h2 className="text-lg font-bold text-white flex items-center gap-2"><Icon name="building-2" className="w-5 h-5 text-teal-400" /> Societies in {activeName}</h2>
        <Link to={`/societies?loc=${activeSlug}`} className="text-sm font-semibold text-teal-400 hover:text-teal-300 inline-flex items-center gap-1">View all <Icon name="arrow-right" className="w-4 h-4" /></Link>
      </div>
          ...
          return (
            <Link key={s.id} to={`/society/${s.slug}`} className="glass-card rounded-xl p-3.5 flex items-center gap-3 hover:border-teal-400/30 transition-all group">
              ...
              <span className="min-w-0">
                ...
                <span className="block text-[11px] text-gray-400 truncate">{verified ? 'Verified · ' : ''}{s.builder}</span>
              </span>
            </Link>
          );
        })}
