...
import { buildShareAlertRecord, shareCriteriaChips, tabMeta } from './alertCriteria.js';
...
/**
 * "Get alerted" card shown in a Share-a-Flat empty state (no flatmates/rooms/groups
 ...
 */
export default function ShareAlertCard({ filters, tab, toast }) {
  const { t } = useTranslation();
  const [mobile, setMobile] = useState(() => myMobile() || '');
  const [channel, setChannel] = useState('whatsapp');
  const [sent, setSent] = useState(false);

  // Per-tab copy so the invitation reads naturally for each share intent.
  const copyMap = { flatmates: t('shareFlat.copyFlatmates'), rooms: t('shareFlat.copyRooms'), groups: t('shareFlat.copyGroups') };
  const record = buildShareAlertRecord(filters, tab);
  const chips = shareCriteriaChips(record);
  const word = tabMeta(tab).word.toLowerCase();

  const handleSubmit = (e) => {
    e.preventDefault();
    if (!/^[6-9]\d{9}$/.test(mobile)) { toast(t('shareFlat.invalidMobile'), 'error'); return; }
    addSavedSearch({ ...record, channel, mobile });
    setSent(true);
    toast(t('shareFlat.alertCreatedChannel', { channel: channel === 'sms' ? 'SMS' : 'WhatsApp' }), 'success');
  };
  ...
  if (sent) {
    return (
      <div className="mt-6 overflow-hidden rounded-2xl border border-teal-500/25 bg-gradient-to-br from-teal-500/10 to-emerald-500/[0.04] p-6 text-center">
        ...
        <p className="text-sm font-semibold text-white">{t('shareFlat.firstInLine')}</p>
        <p className="mx-auto mt-1 max-w-sm text-xs text-gray-400">
          {t('shareFlat.pingPre')} <span className="capitalize text-teal-300">{channel === 'sms' ? 'SMS' : 'WhatsApp'}</span> {t('shareFlat.pingMid')}
          <span className="text-white"> {record.label}</span> {t('shareFlat.pingSuf')}
        </p>
        <Link to="/dashboard#alerts" className="mt-4 inline-flex items-center gap-1.5 rounded-xl border border-teal-400/30 bg-teal-500/10 px-4 py-2 text-sm font-semibold text-teal-200 transition hover:bg-teal-500/20">
          <Icon name="sliders-horizontal" className="h-4 w-4" /> {t('shareFlat.manageAlerts')}
        </Link>
      </div>
    );
  }
      ...
      <div className="p-6">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-sm font-semibold text-white">{t('shareFlat.nothingHereTitle')}</p>
            <p className="mt-0.5 text-xs text-gray-400">
              {copyMap[tab] || copyMap.flatmates} {t('shareFlat.notifyBody', { word })}
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-3">
          {/* Channel choice */}
          <div>
            <p className="mb-1.5 text-[11px] font-medium text-gray-500">{t('shareFlat.notifyOn')}</p>
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
            <div className="relative flex-1 sm:max-w-[240px]">
              ...
              <input
                ...
                inputMode="numeric"
                aria-label={t('shareFlat.ariaMobileAlerts')}
                placeholder={t('shareFlat.mobilePlaceholder')}
            ...
            <button type="submit" className="inline-flex items-center justify-center gap-1.5 rounded-lg bg-teal-500 px-4 py-2 text-sm font-semibold text-ink transition hover:bg-teal-400">
              <Icon name="bell-plus" className="h-4 w-4" /> {t('shareFlat.createAlert')}
            </button>
          </div>
        </form>
      </div>
