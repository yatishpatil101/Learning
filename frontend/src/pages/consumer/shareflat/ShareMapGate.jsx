...
/* Share-a-Flat map is "area-first": the aggregated bubble map is calmest and most
...
export default function ShareMapGate({ areas, selected, onToggle, maxAreas, onSwitchList, kindWord, filters, setF, filtersActive, onClearFilters }) {
  const { t } = useTranslation();
  const kindText = t('shareFlat.kind_' + kindWord);
  ...
  return (
    <div className="glass rounded-2xl border border-white/10 flex items-center justify-center px-6 py-12" style={{ minHeight: 520 }}>
      <div className="w-full max-w-xl text-center">
        ...
        {overLimit ? (
          <>
            <h3 className="text-xl font-bold text-white">{t('shareFlat.overLimitTitle')}</h3>
            <p className="mx-auto mt-2 max-w-md text-sm text-gray-400">
              {t('shareFlat.overLimitPre')} <span className="text-teal-300 font-semibold">{maxAreas}</span> {t('shareFlat.overLimitMid')}{' '}
              <span className="text-white font-semibold">{selected.size}</span>{t('shareFlat.overLimitSuf')}
            </p>
            <div className="mt-5 flex flex-wrap items-center justify-center gap-2">
              {[...selected].map((name) => (
                <button
                  ...
                  aria-label={t('shareFlat.ariaRemoveArea', { name })}
            ...
            >
              <Icon name="list" className="w-4 h-4" /> {t('shareFlat.seeAllAreasList', { count: selected.size })}
            </button>
          </>
        ) : (
          <>
            <h3 className="text-xl font-bold text-white">{t('shareFlat.focusTitle')}</h3>
            <p className="mx-auto mt-2 max-w-md text-sm text-gray-400">
              {t('shareFlat.focusIntroPre')} {kindText} {t('shareFlat.focusIntroSuf')} <span className="text-teal-300 font-semibold">{maxAreas}</span>.
            </p>
            ...
            {visibleAreas.length ? (
              ...
              <div className="mt-6 rounded-xl border border-white/10 bg-white/[0.03] px-4 py-5 text-sm text-gray-400">
                {t('shareFlat.noKindMatchYet', { kind: kindText })}{' '}
                {filtersActive ? (
                  <button type="button" onClick={onClearFilters} className="text-teal-400 hover:text-teal-300 font-medium underline underline-offset-2">{t('shareFlat.clearFilters')}</button>
                ) : t('shareFlat.checkBackSoon')}
              </div>
            )}
            ...
            <div className="mt-7 pt-6 border-t border-white/10 text-left">
              <p className="mb-2 flex items-center gap-1.5 text-xs font-semibold text-gray-300">
                <Icon name="map-pinned" className="w-3.5 h-3.5 text-teal-300" /> {t('shareFlat.orSearchNearPlace')}
              </p>
              <NearPlaceField filters={filters} setF={setF} />
            </div>

            <p className="mt-6 text-xs text-gray-600">
              {t('shareFlat.preferEverythingPre')}{' '}
              <button type="button" onClick={onSwitchList} className="text-teal-400 hover:text-teal-300 font-medium underline underline-offset-2">{t('shareFlat.viewList')}</button> {t('shareFlat.preferEverythingSuf')}
            </p>
          </>
        )}
      </div>
