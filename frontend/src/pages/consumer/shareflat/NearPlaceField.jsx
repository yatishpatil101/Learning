...
/* "Near a Place" for Share-a-Flat — the same proximity model Listings uses, now
...
export default function NearPlaceField({ filters, setF }) {
  ...
  const nearOpts = useMemo(
    () => (filters.near ? [{ value: filters.near, label: filters.nearLabel || t('shareFlat.selectedPlace') }] : []),
  ...
  return (
    <div className="space-y-3">
      <Select
        ...
        options={nearOpts}
        searchable
        placeholder={t('shareFlat.searchPlacePlaceholder')}
        ariaLabel={t('shareFlat.ariaSearchPlace')}
        className="w-full"
      />
      {filters.near ? (
        <div className="space-y-3 pt-1">
          <div className="grid grid-cols-2 gap-1 p-1 rounded-xl bg-white/5 border border-white/10">
            {[['km', 'navigation', t('shareFlat.distance')], ['min', 'clock', t('shareFlat.commuteTime')]].map(([mode, ic, label]) => (
              <button
                key={mode}
                type="button"
          ...
          <div className="flex items-baseline justify-center gap-1.5">
            <input
              ...
              max={25}
              value={nearRadius}
              aria-label={t('shareFlat.ariaRadiusValue')}
            ...
            />
            <span className="text-sm text-gray-400 font-medium">{nearMode === 'km' ? t('shareFlat.kmAway') : t('shareFlat.minCommute')}</span>
          </div>
          ...
          <div>
            <input
              ...
              aria-label={t('shareFlat.ariaSearchRadius')}
              className="w-full accent-teal-400 cursor-pointer"
            />
            <div className="flex justify-between text-[10px] text-gray-500 mt-0.5">
              <span>1 {nearMode === 'km' ? t('shareFlat.unitKm') : t('shareFlat.unitMin')}</span>
              <span>25 {nearMode === 'km' ? t('shareFlat.unitKm') : t('shareFlat.unitMin')}</span>
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {(nearMode === 'km' ? [1, 3, 5, 10, 25] : [5, 10, 15, 20, 25]).map((v) => (
              ...
              >
                {v} {nearMode === 'km' ? t('shareFlat.unitKm') : t('shareFlat.unitMin')}
              </button>
            ))}
          </div>
          ...
          >
            <Icon name="x" className="w-3 h-3" /> {t('shareFlat.clearPlace')}
          </button>
        </div>
      ) : null}
    </div>
