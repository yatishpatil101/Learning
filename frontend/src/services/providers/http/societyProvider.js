/**
 * HTTP society provider. `GET /societies` carries `avgRating`/`reviewCount` per row, so the whole
 * directory's ratings cost one walk of this endpoint: docs/flows/consumer/societies.md
 */
...
/**
 * Well above the eight the rail renders: the badge half of "strongest" is not a sortable column, so
 * the server can only narrow the population. Pinned to the page ceiling, not a second literal.
 */
 ...
 * Warns when {@link MAX_PAGES} caps the walk, because silent truncation resembles a filter failure.
 */
...
/**
 * Listing-bearing societies for client-side strongest ranking; each row carries `listingCount`.
 * Ratings are omitted because the rail does not render them.
 */
...
/**
 * Type-ahead candidates, projected to the six fields the picker renders and with `verified` derived
 * here. **The locality is deliberately not sent** — docs/flows/consumer/societies.md
 */
export async function searchSocieties(query) {
  ...
  return rows.filter((s) => s?.slug).map((s) => {
    ...
    return {
      // The server's UUID, for the one caller not yet moved off ids: the listing wizard still binds
      // a `societyId` into the form. The slug is the key everything else joins on.
      id: s.id,
      slug: s.slug,
      name: s.name || '',
...
/**
 * `GET /societies/{slug}`, not a `q=` search, which would answer with a *near* society. A 404
 * becomes `null` because the hub is reachable from typed URLs; every other failure propagates.
 */
 ...
 * docs/flows/consumer/societies.md
 */
...
/**
 * The same follow list as whole societies. Separate from {@link listFollowedSocieties} because the
 * app-wide follow context only needs a `Set` of slugs, not up to 500 full records.
 */
...
/**
 * Where the caller stands here — one read, four facts. Public and caller-aware, so the hub renders
 * the "claim this society" invitation on first paint without waiting on a sign-in check.
 */
...
/**
 * Ask to be recognised as a resident of one flat. Returns the standing request: the server amends
 * rather than queueing a second, so calling twice leaves one row.
 */
...
/**
 * Verify or reject one request. A 409 is the flat already having a verified resident — a real
 * answer, not a transport failure, so it surfaces as an `ApiError` with a readable message.
 */
...
/**
 * Questions newest first, answers attached. Public, because the person with the most to ask about a
 * building has not moved into it yet.
 */
...
/**
 * Events by when they happen, then notices newest first. `kind` narrows to one; the hub omits it
 * and draws both columns from one read rather than paying two round trips.
 */
...
/**
 * Post an event or a notice. A 403 means the caller has not verified a flat here — the rule, not an
 * error to retry, so it surfaces carrying the server's own sentence.
 */
...
/**
 * Share a tip, a pick or a photo. `photoUrl` must already be a URL — upload through
 * `POST /me/photos` first, or the photo is visible only on the device that shared it.
 */
 ...
 * state the tap intended. Answers `{helpfulCount, helpfulByMe}`, which is what the button draws.
 */
...
/**
 * Every pending proposal plus whether a resident group exists — one read, so the page cannot render
 * half a state. `whatsappJoinUrl` is null without a verified flat here; `whatsappAvailable` is not.
 */
...
/**
 * Residency requests across every society, oldest first. Full-ceiling `size` because this screen has
 * no pager; deciding stays on `decideResidency`, which already owns the one-resident-per-flat rule.
 */
...
/**
 * One request per certificate actually opened, addressed by claim rather than by document — the
 * queue read is deliberately left alone: docs/flows/consumer/societies.md
 */
...
/**
 * Add a society the catalogue does not have. Answers the canonical society either way, so the
 * caller's next move works against the real row rather than a duplicate they did not know they made.
 */
...
/**
 * Confirm a member-added society is real. 409 once verified: the record of who did it is the only
...
/**
 * Duplicate hints for one queued candidate, strongest first — a plain array, because the endpoint
 * answers a handful by construction. One request per candidate an operator actually opens.
 */
...
/**
 * Merges in force, newest first. `unwrapFullPage` rather than a silent `.content`: a console that
 * shows the first twenty merges and calls it the list is worse than one that says so.
 */
 ...
 * several duplicates, so keying on it would resolve silently to the wrong one.
 */
...
/**
 * The body is forwarded as given, not normalised into a full row: the route treats an absent field
 * as unchanged, and `adminNote: ''` (clear) must not be coalesced with no `adminNote` (leave).
 */
