...
export default function KpiStrip({ summary, kpiTrend, cf, netSeries, basis, onEditBasis }) {
  const kpiTiles = [
    { key: 'collected', render: () => <Stat icon="trending-up" bg="bg-emerald-400/15" fg="text-emerald-400" value={fmtINR(summary.income)} label="Rent collected" trend={kpiTrend.income} spark={{ data: cf.incomeData, color: '#34d399' }} className="p-4" /> },
    { key: 'expenses', render: () => <Stat icon="trending-down" bg="bg-rose-400/15" fg="text-rose-400" value={fmtINR(summary.expense)} label="Expenses" spark={{ data: cf.expenseData, color: '#fb7185' }} className="p-4" /> },
    { key: 'net', render: () => <Stat icon="wallet" bg="bg-teal-400/15" fg="text-teal-400" value={fmtINR(summary.net)} label="Net cashflow" trend={kpiTrend.net} spark={{ data: netSeries, color: '#2dd4bf' }} className="p-4" /> },
    {
      key: 'roi',
      render: () => (basis?.purchasePrice && basis?.currentValue) ? (
        <Stat icon="chart-line" bg="bg-teal-400/15" fg="text-teal-400" value={((basis.currentValue - basis.purchasePrice) / basis.purchasePrice * 100).toFixed(1) + '%'} label="Appreciation" className="p-4" />
      ) : (
        <button type="button" onClick={onEditBasis} className="glass-card rounded-2xl p-4 w-full h-full text-left transition hover:bg-white/[0.04] focus:outline-none focus-visible:ring-2 focus-visible:ring-teal-400/60">
          ...
          <p className="text-sm font-semibold text-teal-300">Set up ROI →</p>
          <p className="text-gray-400 text-xs mt-0.5">Add purchase price to track appreciation</p>
        </button>
      ),
    },
  ];
