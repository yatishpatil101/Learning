...
class ListingFoundationTest extends AbstractApiTest {
    ...
    /** Re-checked but still live. {@code address} derives the duplicate key; the rest are the evidence
     *  approval rested on. */
    ...
    /** Each is a filter a buyer trusts, but the listing is still that flat, so the re-check happens in search
    ...
    /** Replacing the photographs, prose and amenities sells a verdict never given about what is now on the
     *  page. No buyer filters on these, so the listing stays findable while the desk looks again. */
    @Test
    ...
    /** PATCH semantics: the natural "the field was present, so re-review" passes every case above while making
     *  an owner who saves a form twice wait on a moderator. */
    @Test
