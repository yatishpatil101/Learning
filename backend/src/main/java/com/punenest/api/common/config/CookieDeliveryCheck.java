 ...
 *
 * <p><strong>The failure this exists to prevent.</strong> {@code punenest_rt} is
 ...
 * a path proxy ({@code punenest.com} serving the app and forwarding {@code /api} here), or they sit
 * on sibling subdomains ({@code www.punenest.com} calling {@code api.punenest.com}) — cross-origin,
 ...
 * {@code punenest.web.public-origin} is unset — dev and tests, where the proxy makes the question
...
public class CookieDeliveryCheck {
     ...
     * the registrable domain takes three labels rather than two, so {@code punenest.netlify.app} and
    ...
    CookieDeliveryCheck(
            @Value("${punenest.web.public-origin:}") String publicOrigin,
    ...
    void verify() {
        ...
        if (publicOrigin.isBlank()) {
            log.info("punenest.web.public-origin is unset — skipping the refresh-cookie delivery "
        ...
        throw new IllegalStateException(
                "Refresh-cookie delivery is impossible for these origins: " + unreachable
                        ...
                        + "punenest_rt cookie to POST /auth/refresh and every session will die at the "
                        ...
                        + "Set punenest.web.public-origin to this service's browser-facing origin, and "
                        + "punenest.web.cors.allowed-origins (WEB_ORIGINS) to the UI's.");
    }
     ...
     * checked above. {@code punenest_rt} is {@code HttpOnly}: the browser delivers it and no script
     * ever touches it, so same-<em>site</em> is the whole requirement. {@code punenest_session} is
    ...
    private void warnIfSessionHintIsUnreadable() {
        ...
        log.warn("Session-hint cookie is unreadable from these origins: {}. They are same-site with "
                ...
                + "punenest_session hint is host-only (__Host- forbids a Domain attribute). Safari's "
    ...
    private static String siteOf(String origin) {
        ...
        if (labels.length < 3) {
            // "localhost", an IP literal, or an already-registrable "punenest.com".
            return host;
        }
