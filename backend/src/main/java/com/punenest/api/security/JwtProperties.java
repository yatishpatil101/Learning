    ...
    public JwtProperties {
        if (accessTtl == null || accessTtl.isNegative() || accessTtl.isZero()) {
            throw new IllegalArgumentException(
                    "punenest.security.jwt.access-ttl must be positive, was " + accessTtl);
        }
        if (refreshTtl == null || refreshTtl.isNegative() || refreshTtl.isZero()) {
            throw new IllegalArgumentException(
                    "punenest.security.jwt.refresh-ttl must be positive, was " + refreshTtl);
        }
        if (refreshTtl.compareTo(accessTtl) < 0) {
            throw new IllegalArgumentException(
                    "punenest.security.jwt.refresh-ttl (" + refreshTtl + ") must not be shorter "
        ...
        if (refreshGrace == null || refreshGrace.isNegative()) {
            throw new IllegalArgumentException(
                    "punenest.security.jwt.refresh-grace must not be negative, was " + refreshGrace);
        }
        if (refreshGrace.compareTo(MAX_GRACE) > 0) {
            throw new IllegalArgumentException(
                    "punenest.security.jwt.refresh-grace (" + refreshGrace + ") exceeds the "
