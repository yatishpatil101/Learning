...
public class TrustedProxyConfig {
    ...
    public WebServerFactoryCustomizer<TomcatServletWebServerFactory> remoteIpCustomizer(
            @Value("${punenest.security.trusted-proxies}") String trustedProxies) {

        String value = trustedProxies == null ? "" : trustedProxies.trim();
        if (value.isEmpty()) {
            throw new IllegalStateException(
                    "punenest.security.trusted-proxies must be set. Use 'none' if this instance is "
        ...
        try {
            ...
            throw new IllegalStateException(
                    "punenest.security.trusted-proxies is not a valid regex: " + value, e);
        }
