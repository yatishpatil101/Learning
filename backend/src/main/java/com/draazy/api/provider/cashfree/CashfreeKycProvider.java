 ...
 * only thing tying that webhook back to a Draazy user. Secure ID has no polling endpoint, so
 * losing the ref means losing the verification.
 *
 * <p>Only wired when {@code draazy.providers.cashfree.enabled=true}; otherwise {@code
 * MockKycProvider} stands in and the badge flow is fully demoable with no vendor account.
 *
 * <p><strong>Unverified against a live account.</strong> Draazy has no Cashfree credentials yet, so
...
class CashfreeKycProvider implements KycProvider {
    ...
    CashfreeKycProvider(
            ...
            @org.springframework.beans.factory.annotation.Value(
                    "${draazy.providers.cashfree.kyc-redirect-url:https://draazy.in/verify/complete}")
            String redirectUrl) {
    ...
    public KycSession start(String userId) {
        ...
        String verificationId = "dz_" + UUID.randomUUID();
