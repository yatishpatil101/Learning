...
import com.draazy.api.common.config.CorsConfig;
 ...
 * {@code POST /auth/refresh} (see {@link com.draazy.api.security.SecurityConfig}), and it is a
 ...
 * {@link com.draazy.api.common.config.CookieDeliveryCheck} blesses — a frontend on
 * {@code www.draazy.in} talking to an API on {@code api.draazy.in}, which is the only shape in
 ...
 * them can run
 *
 * <pre>{@code fetch('https://api.draazy.in/api/auth/refresh', { credentials: 'include' })}</pre>
 *
...
public class RefreshOriginGate {
    ...
    public void check(HttpServletRequest request) {
        ...
        log.warn("Refusing to rotate a refresh token for a {} request from origin {}: not an origin "
                ...
                site, origin == null ? "<absent>" : origin, "draazy.web.cors.allowed-origins");
